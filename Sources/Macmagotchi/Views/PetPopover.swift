        ...
        ZStack {
            ...
            if pet.isOnboarding {
                ...
                VStack(spacing: 15) {
                    ...
                    VStack(spacing: 10) {
                        StatRow(
                            icon: "fork.knife", label: settings.t("hunger"), value: pet.hunger,
                            tint: .orange, foreground: settings.theme.foreground, secondary: settings.theme.secondary
                        )
                        StatRow(
                            icon: "face.smiling", label: settings.t("mood"), value: pet.mood,
                            tint: .pink, foreground: settings.theme.foreground, secondary: settings.theme.secondary
                        )
                        StatRow(
                            icon: "bolt.fill", label: settings.t("energy"), value: pet.energy,
                            tint: .yellow, foreground: settings.theme.foreground, secondary: settings.theme.secondary
                        )
                        StatRow(
                            icon: "heart.fill", label: settings.t("affection"), value: min(100, pet.affection),
                            tint: .mint, foreground: settings.theme.foreground, secondary: settings.theme.secondary
                        )
                    }.padding(14).background(settings.theme.panel, in: RoundedRectangle(cornerRadius: 18))
                    if pet.isFocusing {
                        VStack(alignment: .leading, spacing: 6) {
                            HStack {
                                Text(timeString(pet.secondsRemaining)).font(.title2.monospacedDigit())
                                Spacer()
                                Text("Focus").font(.caption.weight(.semibold))
                            }
                            ProgressView(value: Double(pet.secondsRemaining), total: Double(pet.focusTotalSeconds))
                                .tint(.mint)
                        }.foregroundStyle(settings.theme.foreground)
                    }
                    HStack(spacing: 8) {
                        ForEach(PetTask.allCases.sorted { settings.minutes($0) < settings.minutes($1) }) { task in
                            ActionButton(
                                settings.t(task.titleKey), icon: task.icon, tint: task.tint,
                                foreground: settings.theme.foreground
                            ) { pet.begin(task, minutes: settings.minutes(task)) }
                        }
...
struct WelcomeView: View {
    ...
    var body: some View {
        VStack(spacing: 18) {
            VStack(spacing: 6) {
                Text(settings.t("welcome"))
                    .font(.system(size: 27, weight: .bold, design: .rounded))
                    .foregroundStyle(settings.theme.foreground)
                Text(settings.t("chooseFriend"))
                    .font(.subheadline)
            ...
            HStack(spacing: 10) {
                ForEach(PetKind.allCases, id: \.self) { option in
                    Button { kind = option } label: {
                        ...
                        .background(
                            kind == option ? option.color.opacity(0.58) : .white.opacity(0.10),
                            in: RoundedRectangle(cornerRadius: 16)
                        )
                        .overlay(
                            RoundedRectangle(cornerRadius: 16)
                                .stroke(.white.opacity(kind == option ? 0.8 : 0), lineWidth: 1)
                        )
                }
            }
            TextField(settings.t("namePlaceholder"), text: $name)
                .textFieldStyle(.plain)
                .padding(12)
                .background(settings.theme.panel, in: RoundedRectangle(cornerRadius: 12))
                .foregroundStyle(settings.theme.foreground)
...
struct ActionButton: View {
    ...
    init(_ title: String, icon: String, tint: Color, foreground: Color, action: @escaping () -> Void) {
        self.title = title
        self.icon = icon
        self.tint = tint
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Menu {
                ...
                SettingsRow(
                    icon: "globe", title: settings.t("language"), value: settings.language.title
                )
            ...
            ForEach(PetTask.allCases) { task in
                HStack {
                    ...
                    Spacer()
                    Stepper(
                        "\(settings.minutes(task)) min",
                        value: settings.minutesBinding(task),
                        in: 5...480,
                        step: 5
                    ).labelsHidden()
